...
function run (args) {
  ...
  while (arg = args.shift()) {
    ...
    } else if (arg === "--restart-delay" || arg === "-r") {
      restartDelay = parseInt(args.shift());
...
function help () {
  print
    ...
    ("")
    ("  -r|--restart-delay <milliseconds>")
    ("    How long to wait before starting after a crash. Use to reduce cpu")
    ("    load on crashing programs.")
    ("    Defaults to 0 ms.")
    ("")
...
function startProgram (prog, exec) {
  ...
  child.addListener("exit", function (code) {
    ...
    if (!start_queued) {
      start_queued = true;
      setTimeout(function() { startProgram(prog, exec); }, restartDelay);
    }
  });
}
